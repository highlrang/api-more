...
import coding.json.practice.batch.jobs.etc.ItemListProcessor;
import coding.json.practice.batch.jobs.etc.JpaItemListWriter;
...
public class JpaItemListWriterJobConfig {
    ...
    private ItemProcessor<Sales, List<Tax>> processor(){
        return new ItemListProcessor(); // custom
    }

    private JpaItemListWriter<Tax> writerList() {
        // List<Tax>를 전달 받음 > 두 개의 Sales를 통해 6개의 tax list가 write됨
        JpaItemWriter<Tax> writer = new JpaItemWriter<>();
        writer.setEntityManagerFactory(entityManagerFactory);

        return new JpaItemListWriter<>(writer); // custom
    }
}
